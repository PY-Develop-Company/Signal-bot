...
class MainAnalizer(Analizer):
    ...
    def analize_func(self, df) -> (bool, Signal, int, str):
        interval_td = df.datetime[0] - df.datetime[1]
        interval = interval_convertor.datetime_to_interval(interval_td)
        ...
        has_signal = main_signal[1][0] >= self.successful_indicators_count and indicators_signals[0][0].type == \
                     main_signal[0]
    ...
    def analize(self, df) -> (bool, Signal, int, str):
        has_signal, signal, ind_count, debug_text = self.analize_func(df)
        print("debug:", debug_text)
